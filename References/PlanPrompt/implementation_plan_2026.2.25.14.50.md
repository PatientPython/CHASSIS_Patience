...
**Location Requirement**: All hook scripts MUST be completely transferred to and installed in the user's global directory (`~/.claude/hooks/` on macOS/Linux or `%USERPROFILE%\.claude\hooks\` on Windows) to make them globally effective across all projects.
...
**Location Requirement**: All skills MUST be completely transferred to and installed in the user's global directory (`~/.claude/skills/` on macOS/Linux or `%USERPROFILE%\.claude\skills\` on Windows) so they can be accessed anywhere.
...
- **Behavior Requirements (Handling Parallel Realities)**:
  ...
  - **Execution Flow**:
    ...
    4. When this new session starts on the new branch, the `SessionStart` hook fires automatically. It realizes this is a brand new Session ID and natively mounts/saves it to the new Git sub-branch in `.claude/session-git-map.json`.You need to handle your hooks properly here to accurately achieve this effect.
...
1. **Config Hook File**: `~/.claude/settings.json` (or `%USERPROFILE%\.claude\settings.json`) must be standard JSON defining the "hooks" object dictionary, so that hooks take effect universally at the user level.
