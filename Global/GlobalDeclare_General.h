...
typedef struct {
    /*底盘相关*/
    _ChassisCtrlData_DefaultConfig_StructTypeDef
        STCH_Default;  // 底盘默认可配置控制变量结构体
    _ChassisCtrlData_ForceConfig_StructTypeDef
        STCH_Force;  // 底盘强制可配置控制变量结构体
...
#define RCJoyStick_UpTH             \
    ((uint16_t)RCChannelValue_Max - \
     100)  // 遥控器摇杆上阈值，Y轴超过该值认为摇杆在上
#define RCJoyStick_DownTH           \
    ((uint16_t)RCChannelValue_Min + \
     100)  // 遥控器摇杆下阈值，Y轴低于该值认为摇杆在下
#define RCJotStick_LeftTH           \
    ((uint16_t)RCChannelValue_Min + \
     100)  // 遥控器摇杆左阈值，X轴低于该值认为摇杆在左
#define RCJoyStick_RightTH          \
    ((uint16_t)RCChannelValue_Max - \
     100)  // 遥控器摇杆右阈值，X轴超过该值认为摇杆在右
#define RCRoller_UpTH               \
    ((uint16_t)RCChannelValue_Min + \
     100)  // 遥控器拨轮上阈值，拨轮低于该值认为拨轮在上
#define RCRoller_DownTH             \
    ((uint16_t)RCChannelValue_Max - \
...
#define HubMotorRx_fpsMinTH \
    700  // 轮毂电机接收帧率最小阈值，低于该值可能是电机断开
...
#define ESC_C620AmpereToCurrent \
    (1 / 20.0f * 16384.0f)  // C620电调电流转换系数，单位：映射电流值/A

// #define Motor_3508ReductionRatio        3591.0f/187.0f
// //3508电机减速比(本体的减速箱)
#define Motor_3508GearboxReductionRatio \
    268.0f / 17.0f  // 3508电机外接齿轮箱减速比
#define Motor_3508MaxCurrent \
    ESC_C620MaxCurrent  // 3508电机（连在C620电调上）的最大电流值
#define Motor_3508MinCurrent \
    ESC_C620MinCurrent  // 3508电机（连在C620电调上）的最小电流值
#define Motor_3508AmpereToCurrent \
    ESC_C620AmpereToCurrent  // 3508电机（连在C620电调上）电流转换系数，单位：映射电流值/A
#define Motor_3508Kt 0.3f  // 3508电机转矩常数，单位：Nm/A

#define Motor_MG8016Ei6MaxTorque \
    37.0f  // 瓴控的MG8016E-i6电机最大转矩，单位：Nm
