...
void ChModeControl_FreeMode_RCControl(void)
{
    ...
    int8_t Backward = -1;
    int8_t Brake = 0;
    bool IsDirectionInvert = false; //是否反转移动方向的标志位（如果反转，那么前进变后退，后退变前进）
    float StillVelTH     = 0.4f;    //静止速度阈值
    float VelocityTarget = 1.8f;    //目标速度
    float Acc_Moving     = 2.0f;    //运动加速度（理论值，实际上会更小一些）
    float Acc_Brake      = 10.0f;   //刹车加速度（理论值，实际上会更小一些）
    float VelDesMin      = 1.5f;    //目标速度最小值
    float VelMovingChangeRateMin = 0.1f;  //速度变化最小值
    float VelMovingChangeRateMax = 0.65f; //速度变化最大值
    float VelBrakingChangeRateMax = 1.5f; //刹车时速度变化最大值
    float BrakeVelLimitTH = 0.9f; //刹车时目标速度限制阈值

    int8_t MoveDirection = Brake;

    float VelFBNow   = GSTCH_Data.VelFB; //当前实际速度
    float VelDesPre  = GST_RMCtrl.STCH_Default.VelDes; //上一个目标速度
    float VelDesNext = 0.0f;         //下一个目标速度
    ...
    //待优化：把车修好之后可以把Back的限幅改成MyAbs，然后在前面加上负号，这个就更好看一些（当然还是要调试避免写出bug）
    /*速度的赋值*/
    if(MoveDirection == Forward && IsDirectionInvert == false || MoveDirection == Backward && IsDirectionInvert == true)
    {
        if(VelFBNow <= -StillVelTH)
        {
            VelDesNext = VelFBNow + VelBrakingChangeRateMax;
        }
        else
        {
            VelDesNext = StepChangeValue(VelDesPre, VelocityTarget, Acc_Moving * GCH_TaskTime);    //逐步改变目标速度
            VelDesNext = Limit(VelDesNext, VelFBNow + VelMovingChangeRateMin, VelFBNow + VelMovingChangeRateMax); //限制目标速度变化范围
            VelDesNext = Limit(VelDesNext, VelDesMin, VelocityTarget);                                //限制目标速度最小最大值
        }

    }

    else if(MoveDirection == Backward && IsDirectionInvert == false || MoveDirection == Forward && IsDirectionInvert == true)
    {
        if(VelFBNow >= StillVelTH)
        {
            VelDesNext = VelFBNow - VelBrakingChangeRateMax;
        ...
        {
            VelDesNext = StepChangeValue(VelDesPre, -VelocityTarget, Acc_Moving * GCH_TaskTime); //逐步改变目标速度
            VelDesNext = Limit(VelDesNext, VelFBNow - VelMovingChangeRateMax, VelFBNow - VelMovingChangeRateMin);//限制目标速度变化范围
            VelDesNext = Limit(VelDesNext, -VelocityTarget, -VelDesMin);                                 //限制目标速度最小最大值
        }
    }

    else if (MoveDirection == Brake)
    {
        VelDesNext = StepChangeValue(VelDesPre, 0.0f, Acc_Brake * GCH_TaskTime);
        if(VelFBNow > BrakeVelLimitTH)//如果当前速度是正的
        {
            VelDesNext = Limit(VelDesNext, VelFBNow - VelBrakingChangeRateMax, VelFBNow);   //限制目标速度变化范围
            VelDesNext = Limit(VelDesNext, 0.0f, 0.8f);  //限制目标速度最小最大值
        }
        else if(VelFBNow < -BrakeVelLimitTH)//如果当前速度是负的
        {
            VelDesNext = Limit(VelDesNext, VelFBNow, VelFBNow + VelBrakingChangeRateMax);   //限制目标速度变化范围
            VelDesNext = Limit(VelDesNext, -0.8f, 0.0f); //限制目标速度最小最大值
        }
    }

    /*没停下，目标位移等于实际位移*/
    if(MyAbsf(VelFBNow) > StillVelTH)
    {
    ...
    {
        GST_RMCtrl.STCH_Default.YawAngleVelDes = 90.0f;
    }
