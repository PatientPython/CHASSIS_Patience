...
*/
uint16_t _CRC16_GetCheckSum(uint8_t *pchMessage,uint32_t dwLength,uint16_t wCRC)
{
    if (pchMessage == 0) 
    {return 0xFFFF;}
...
void _CRC16_Append(uint8_t *pchMessage,uint32_t dwLength)
{
    ...
    wCRC = _CRC16_GetCheckSum((uint8_t*)pchMessage, dwLength-2, CRC16_InitValue);
...
void USART6_DMA_printf(const char* fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    uint8_t USART6_DMA_Len = (uint8_t)vsprintf(USART6_DMA_Buf, fmt, ap);//将格式化数据打到USART6_DMA_Buf字符串上，返回格式化长度
    ...
    USART6_TX_STREAM->M0AR = (uint32_t)USART6_DMA_Buf;      //设置当前待发数据基地址:Memory0Target
    USART6_TX_STREAM->NDTR = (uint32_t)USART6_DMA_Len;      //设置当前待发的数据的数量:Number of Data units to be Transferred
...
void UA1Rx_ReceiverDataProcess(void)
{
    /**************************************************遥控器数据解码**************************************************/
    /*右摇杆*/
    GST_Receiver.ST_RC.JoyStickR_X = (UA1RxDMAbuf[0]| (UA1RxDMAbuf[1] << 8)) & 0x07ff;//Channe0——水平通道
    GST_Receiver.ST_RC.JoyStickR_Y = ((UA1RxDMAbuf[1] >> 3) | (UA1RxDMAbuf[2] << 5)) & 0x07ff;//Channe1——垂直通道
    /*左摇杆*/
    GST_Receiver.ST_RC.JoyStickL_X = ((UA1RxDMAbuf[2] >> 6) | (UA1RxDMAbuf[3] << 2) | (UA1RxDMAbuf[4] << 10)) & 0x07ff;//Channe2——水平通道
    GST_Receiver.ST_RC.JoyStickL_Y = ((UA1RxDMAbuf[4] >> 1) | (UA1RxDMAbuf[5] << 7)) & 0x07ff;//Channe3——垂直通道
    /*拨盘*/
    GST_Receiver.ST_RC.Roller = (UA1RxDMAbuf[16] | UA1RxDMAbuf[17] << 8) & 0x07ff;//Channe4——垂直通道
    /*左3位拨杆*/
    GST_Receiver.ST_RC.Level_L = ((UA1RxDMAbuf[5] >> 4)& 0x000C) >> 2;//(上——1；中——3；下——2)
    /*右3位拨杆*/
    GST_Receiver.ST_RC.Level_R = ((UA1RxDMAbuf[5] >> 4)& 0x0003);//(上——1；中——3；下——2)

    /**************************************************鼠标数据解码***************************************************/
    GST_Receiver.ST_Mouse.X = UA1RxDMAbuf[6]  | (UA1RxDMAbuf[7] << 8);// X坐标
    GST_Receiver.ST_Mouse.Y = UA1RxDMAbuf[8]  | (UA1RxDMAbuf[9] << 8);//Y坐标
    GST_Receiver.ST_Mouse.Z = UA1RxDMAbuf[10] | (UA1RxDMAbuf[11] << 8);//Z坐标
    GST_Receiver.ST_Mouse.Left  = UA1RxDMAbuf[12];//左键状态（1：按下；0：没按下）
    GST_Receiver.ST_Mouse.Right = UA1RxDMAbuf[13];//右键状态（1：按下；0：没按下）

    /**************************************************键盘数据解码***************************************************/
    GST_Receiver.usKeyboard = UA1RxDMAbuf[14] | (UA1RxDMAbuf[15] << 8);//键盘值
}
...
void UA2Rx_IMU1DataProcess(void)
{
    if (__UA2Rx_IsVerifySuccess()) //数据校验，包括帧头和CRC
    {
        ...
        float TempCoe = arm_cos_f32(GstGB_IMU1.ST_Rx.PitchAngle*A2R);//处理YawSpeed时需要除的一个值
...
void UA2Tx_SendDataToIMU1(void)
{
    ...
    GstGB_IMU1.ST_Tx.ReloadStatus   = (uint8_t)(false); //原本是之前赛季的补弹标志位，由于规则改动，无需补单功能，直接发false就行
    ...
    USART2_TX_STREAM->M0AR = (uint32_t)&GstGB_IMU1.ST_Tx;   //设置当前待发数据基地址:Memory0 tARget
    USART2_TX_STREAM->NDTR = (uint32_t)UA2TxDMAbuf_LEN;     //设置当前待发的数据的数量:Number of Data units to be TRansferred
...
void USART3_DMA_printf(const char* fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    uint8_t USART3_DMA_Len = (uint8_t)vsprintf(USART3_DMA_Buf, fmt, ap);//将格式化数据打到sz字符串上，返回格式化长度
    ...
    USART3_TX_STREAM->M0AR = (uint32_t)USART3_DMA_Buf;      //设置当前待发数据基地址:Memory0Target
    USART3_TX_STREAM->NDTR = (uint32_t)USART3_DMA_Len;      //设置当前待发的数据的数量:Number of Data units to be Transferred
...
void UA4Tx_SendDataToIMU2(void)
{
    ...
    GstCH_IMU2.ST_Tx.RestartFlag       = (uint8_t)GFCH_IMU2Restart;  //底盘云控重启标志位（待优化：但是经过测试IMU2好像不会重启，可能是IMU2那边没有设置好，后面可以看看改一下）
    GstCH_IMU2.ST_Tx.ReloadStatus      = (uint8_t)(false);           //待优化：原本是之前赛季的补弹标志位，由于规则改动，无需补弹功能，直接发false就行，后续可以删掉，记得IMU2里面也要一起删
    ...
    UART4_TX_STREAM->M0AR = (uint32_t)&GstCH_IMU2.ST_Tx;    //设置当前待发数据基地址:Memory0 Target
    UART4_TX_STREAM->NDTR = (uint32_t)UA4TxDMAbuf_LEN;      //设置当前待发的数据的数量:Number of Data units to be Transferred
